...
function closeNav() {
  ...
  <br>
  <br>
     <h2 style="text-align:center;">LAST DAY ATTENDENCE</h2>
  <br>
  <br>
  ...
  <?php
  $date4=date('Y-m-d');
  $s_date=date('Y-m-d', strtotime($date4. ' - 2 days'));
  $remind=date('Y-m-d', strtotime($date4. ' - 1 days'));
  $sql="SELECT * FROM `$table_name12` WHERE a_date BETWEEN '$s_date' AND '$remind'";
