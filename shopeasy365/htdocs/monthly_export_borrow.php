...
<?php
  ...
  $date3=date('Y-m-d', strtotime($date2. ' + 30 days'));
  ...
  //echo $date2;
  //echo $date3;
  $sql33="SELECT * FROM $table_name where BDATE between '$date2' and '$date1'";
 ...
 if(mysqli_num_rows($result) > 0)
 {
  ...
  while($row = mysqli_fetch_array($result))
  {
   $output .= '
    <tr>
                         <td>'.$i.'</td>
                         <td>'.$row["BCUSTOMER_NAME"].'</td>
                         <td>'.$row["BDATE"].'</td>
                         <td>'.$row["BPRODUCTS"].'</td>
                         <td>'.$row["BTOTALBILL_AMOUNT"].'</td>
                         <td>'.$row["BGAIN_AMOUNT"].'</td>
                         <td>'.$row["BBORROWING_REASON"].'</td>
                         <td>'.$row["bmobile_no"].'</td>
                         <td>'.$row["BBORROWING_AMOUNT"].'</td>
